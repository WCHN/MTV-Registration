    ...
    */
    ...
    for(y=1.0; y<dg[1]-s[1]; y+=s[1])
    {
        ...
        for(x=1.0; x<dg[0]-s[0]; x+=s[0])
        {
            ...
            if (yp>=1.0 && yp<df[1] && xp>=1.0 && xp<df[0])
            {
                float vf;
                int   ivf, ivg;
                vf  = (samp(df, f, xp,yp,zp)/255)*31;
                ivf = floor(vf);
                ivg = floor((samp(dg, g, rx,ry,rz)/255)*31 + 0.5);
                H[ivf+ivg*32] += (1-(vf-ivf));
                if (ivf<31)
                    H[ivf+1+ivg*32] += (vf-ivf);
            }
        }
    }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (mxGetNumberOfDimensions(prhs[0]) != 2) mexErrMsgTxt("Wrong number of dims (1).");
    dimsg = mxGetDimensions(prhs[0]);    
            
    if (!mxIsNumeric(prhs[1]) || !mxIsUint8(prhs[1]) || mxIsComplex(prhs[1]))
        mexErrMsgTxt("Wrong sort of data (2).");
    if (mxGetNumberOfDimensions(prhs[1]) != 2) mexErrMsgTxt("Wrong number of dims (2).");
    ...
    plhs[0] = mxCreateDoubleMatrix(32,32,mxREAL);
