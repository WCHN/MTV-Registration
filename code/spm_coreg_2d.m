function x = spm_coreg_2d(varargin)
% Adapted spm_coreg, which does between modality coregistration using
% information theory, to work in 2D.
...
for k=1:numel(VF)
    ...
    xk  = flags.params(:);
    for samp=flags.sep(:)'
        xk     = my_spm_powell(xk(:), xi,sc,[],mfilename,VG,VFk,samp,flags.cost_fun,flags.fwhm);
        x(k,:) = xk(:)';
    end
    if flags.graphics
...
%==========================================================================
function o = optfun(x,VG,VF,s,cf,fwhm)
% The function that is minimised when doing MI registration.
if nargin<6, fwhm = [7 7];   end
if nargin<5, cf   = 'mi';    end
if nargin<4, s    = [1 1]; end
...
x = [x(1) x(2) 0 0 0 x(3)];
H = spm_hist2_2d(VG.uint8,VF.uint8, VF.mat\spm_matrix(x(:)')*VG.mat);
...
if size(V.pinfo,2)==1 && V.pinfo(1) == 2
    ...
    mx = -Inf; mn =  Inf;
    for p=1:V.dim(3)
%         img = spm_slice_vol(V,spm_matrix_rigid_2d([0 0 p]),V.dim(1:2),1);
        img = V.private.dat(:,:,:);
...
H   = spm_hist2_2d(VG.uint8,VF.uint8,VF.mat\VG.mat);
tmp = log(H+1);
